...
```{r,echo=FALSE, message=FALSE, include=FALSE}
# activate if needed or running for first time
...
ggplot(annot_complete, aes(x = Sex, y = percent, fill = Group)) +
  ...
  scale_fill_manual(breaks = c("Tumor", "TANM"),
                     values = c("#A52A2A", "#4682B4")) +
  stat_compare_means(comparisons = list(c("Tumor", "TANM")), method = "t.test", label = "p.format") +
  theme_classic() +
  facet_wrap(~ Group)
...
# test only highly abundant peptides
 
```{r}
sequences_cut <- dplyr::select(sequences, contains("KB")) %>%
  mutate(mean_intens = rowMeans(., na.rm = TRUE)) %>%
  slice_max(order_by = mean_intens, n = round((nrow(.)*0.2), digits = 0))

semi_percent <- data.frame(total = colSums(dplyr::select(sequences_cut, contains("KB")), na.rm = TRUE)) %>%
  rownames_to_column(var = "Sample") %>%
  mutate(semi = colSums(dplyr::select(semi_data_raw, contains("KB")), na.rm = TRUE)) %>%
  mutate(percent = (semi / total)*100) %>%
  left_join(dplyr::select(annot, "Sample", "Group"), by = "Sample")
...
ggplot(annot_plot, aes(x = Group, y = percent, color = Group)) +
  geom_boxplot(outlier.shape = NA) +
  scale_color_manual(breaks = c("Tumor", "TANM"),
                     values = c("red", "blue")) +
  geom_jitter(alpha = 0.3) +
